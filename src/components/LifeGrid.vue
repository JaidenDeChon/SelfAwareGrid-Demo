 ...
 * gone quiet, and start it moving again, rather than blanking the hero with a fresh seed.
 */
...
function tick (): void {
     ...
     * the hero looks frozen even though the rules are being followed exactly.
     */
    quietGenerations = churn() < cells.length * 0.04 ? quietGenerations + 1 : 0;

    if (population() < cells.length * 0.04 || generation > 600) {
        seed();
